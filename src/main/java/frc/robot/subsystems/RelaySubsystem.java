...
public class RelaySubsystem extends SubsystemBase {
     ...
     */
    private Logger logger = Logger.getLogger("RelaySubsystem");
    ...
    private Relay getRelay(int portNumber) {
        switch (portNumber) {
            ...
            default: {
                logger.severe(String.format("Relay at requested port number %d does not exist in code", portNumber));
                return null;
            }
        }
    }
    ...
    public void toggleRelay(int portNumber) {
        ...
        if (relay == null) {
            logger.severe(String.format("Null relay on port number %d", portNumber));
            return;
        }
